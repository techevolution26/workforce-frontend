...
export default function WorkerAssignmentsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((assignment) => {
      const title = assignment.job?.title?.toLowerCase() || "";
      const location = assignment.job?.location_text?.toLowerCase() || "";
      const status = assignment.status.toLowerCase();
      const source = assignment.source.toLowerCase();
  ...
  return (
    <div className="space-y-6">
      <Card>
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-neutral-600">
              Invitations are employer-issued. Applications are your own submissions.
            </p>
          </div>
        ...
        <div className="mt-4">
          <Input
            placeholder="Search by job title, location, assignment id, status, or source"
            value={query}
